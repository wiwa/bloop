...
private[dap] object Endpoints {
  ...
  final class Event[A <: DebugEvent: ClassTag](name: String) {
    def first(implicit proxy: Proxy): Task[A] = all.firstL

    def all(implicit proxy: Proxy): Observable[A] = {
      proxy.events
