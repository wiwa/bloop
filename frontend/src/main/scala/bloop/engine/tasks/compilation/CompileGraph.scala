...
object CompileGraph {
  ...
  def traverse(
      dag: Dag[Project],
      setup: Project => Task[CompileBundle],
   ...
   */
  def setupAndDeduplicate(project: Project, setup: Project => Task[CompileBundle])(
      compile: CompileBundle => CompileTraversal
  ): CompileTraversal = {
    setup(project).flatMap { bundle =>
  ...
  private def normalTraversal(
      dag: Dag[Project],
      computeBundle: Project => Task[CompileBundle],
      compile: Inputs => Task[Compiler.Result],
      logger: Logger
  ): CompileTraversal = {
      ...
      tasks.get(dag) match {
        ...
        case None =>
          val task: Task[Dag[PartialCompileResult]] = dag match {
            case Leaf(project) =>
              setupAndDeduplicate(project, computeBundle) { bundle =>
              ...
              setupAndDeduplicate(project, computeBundle) { bundle =>
                ...
                Task.gatherUnordered(downstream).flatMap { dagResults =>
                  ...
                  if (failed.isEmpty) {
                    ...
                    val indexDirs =
                      project.compilationClasspath.iterator.filter(_.isDirectory).zipWithIndex.toMap
                    val dependentStore = {
                      val transitiveStores = results.flatMap(r =>
